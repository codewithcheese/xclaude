...
# ── Usability ──
__deno="${HOME}/.deno/bin/deno"

t "deno: deno --version"
expect_success "runs" tc_sandboxed "$__deno" --version

t "deno: deno eval"
expect_success "eval" tc_sandboxed "$__deno" eval "console.log('sandbox ok')"

t "deno: deno run script"
printf 'const x = 1 + 2;\nconsole.log("result:", x);\n' > "${PROJECT_DIR}/deno-test.ts"
expect_success "deno run" tc_sandboxed "$__deno" run "${PROJECT_DIR}/deno-test.ts"
rm -f "${PROJECT_DIR}/deno-test.ts"

t "deno: deno init"
expect_success "deno init" tc_sandboxed "$__deno" init "${PROJECT_DIR}/deno-test-proj"
expect_success "deno.json" tc_sandboxed test -f "${PROJECT_DIR}/deno-test-proj/deno.json"
