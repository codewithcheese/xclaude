...
# ── Toolchain tests ──────────────────────────────────────────
# For each toolchain, assemble a profile with it enabled, create
# fixture directories, and verify the sandbox grants access.
echo "=== Toolchains ==="

# Helper: assemble a profile with a given .xclaude config and
# return a sandboxed runner function.
__tc_profile_path=""
tc_sandboxed() {
  sandbox-exec \
    -D "PROJECT_DIR=${PROJECT_DIR}" \
...
tc_test() {
  local tc_name="$1"
  local config="tool ${tc_name}"

  # Write .xclaude and pre-trust it
  echo "$config" > "${PROJECT_DIR}/.xclaude"
  __xclaude_trust "${PROJECT_DIR}/.xclaude"
...
# ── node ──
tc_test "node"
mkdir -p "${HOME}/.nvm/versions" "${HOME}/.npm"
__fixtures_created+=("${HOME}/.nvm/versions" "${HOME}/.nvm" "${HOME}/.npm")

t "node: read ~/.nvm"
echo "fixture" > "${HOME}/.nvm/test"
__fixtures_created+=("${HOME}/.nvm/test")
expect_success "allowed" tc_sandboxed cat "${HOME}/.nvm/test"

t "node: write ~/.npm"
expect_success "allowed" tc_sandboxed touch "${HOME}/.npm/test-write"
rm -f "${HOME}/.npm/test-write"

t "node: ~/.ssh still blocked"
expect_fail "blocked" tc_sandboxed cat "${HOME}/.ssh/known_hosts"

# ── uv ──
tc_test "uv"
mkdir -p "${HOME}/.cache/uv" "${HOME}/.local/share/uv"
__fixtures_created+=("${HOME}/.cache/uv" "${HOME}/.local/share/uv")

t "uv: read ~/.cache/uv"
echo "fixture" > "${HOME}/.cache/uv/test"
__fixtures_created+=("${HOME}/.cache/uv/test")
expect_success "allowed" tc_sandboxed cat "${HOME}/.cache/uv/test"

t "uv: write ~/.cache/uv"
expect_success "allowed" tc_sandboxed touch "${HOME}/.cache/uv/test-write"
rm -f "${HOME}/.cache/uv/test-write"

t "uv: ~/.ssh still blocked"
expect_fail "blocked" tc_sandboxed cat "${HOME}/.ssh/known_hosts"

# ── rust ──
tc_test "rust"
mkdir -p "${HOME}/.cargo/bin" "${HOME}/.cargo/registry" "${HOME}/.rustup"
__fixtures_created+=("${HOME}/.cargo/bin" "${HOME}/.cargo/registry" "${HOME}/.cargo" "${HOME}/.rustup")

t "rust: read ~/.cargo"
echo "fixture" > "${HOME}/.cargo/test"
__fixtures_created+=("${HOME}/.cargo/test")
expect_success "allowed" tc_sandboxed cat "${HOME}/.cargo/test"

t "rust: read ~/.rustup"
echo "fixture" > "${HOME}/.rustup/test"
__fixtures_created+=("${HOME}/.rustup/test")
expect_success "allowed" tc_sandboxed cat "${HOME}/.rustup/test"
...
t "rust: ~/.ssh still blocked"
expect_fail "blocked" tc_sandboxed cat "${HOME}/.ssh/known_hosts"

# ── go ──
tc_test "go"
mkdir -p "${HOME}/go/bin" "${HOME}/go/pkg" "${HOME}/.cache/go-build"
__fixtures_created+=("${HOME}/go/bin" "${HOME}/go/pkg" "${HOME}/go" "${HOME}/.cache/go-build")

t "go: read ~/go"
echo "fixture" > "${HOME}/go/test"
__fixtures_created+=("${HOME}/go/test")
expect_success "allowed" tc_sandboxed cat "${HOME}/go/test"

t "go: write ~/go/pkg"
expect_success "allowed" tc_sandboxed touch "${HOME}/go/pkg/test-write"
rm -f "${HOME}/go/pkg/test-write"

t "go: ~/.ssh still blocked"
expect_fail "blocked" tc_sandboxed cat "${HOME}/.ssh/known_hosts"

# ── python ──
tc_test "python"
mkdir -p "${HOME}/.pyenv/shims" "${HOME}/.pyenv/versions"
__fixtures_created+=("${HOME}/.pyenv/shims" "${HOME}/.pyenv/versions" "${HOME}/.pyenv")

t "python: read ~/.pyenv"
echo "fixture" > "${HOME}/.pyenv/test"
__fixtures_created+=("${HOME}/.pyenv/test")
expect_success "allowed" tc_sandboxed cat "${HOME}/.pyenv/test"
...
t "python: ~/.ssh still blocked"
expect_fail "blocked" tc_sandboxed cat "${HOME}/.ssh/known_hosts"

# ── deno ──
tc_test "deno"
mkdir -p "${HOME}/.deno/bin"
__fixtures_created+=("${HOME}/.deno/bin" "${HOME}/.deno")

t "deno: read ~/.deno"
echo "fixture" > "${HOME}/.deno/test"
__fixtures_created+=("${HOME}/.deno/test")
expect_success "allowed" tc_sandboxed cat "${HOME}/.deno/test"

t "deno: write ~/.deno"
expect_success "allowed" tc_sandboxed touch "${HOME}/.deno/test-write"
rm -f "${HOME}/.deno/test-write"

t "deno: ~/.ssh still blocked"
expect_fail "blocked" tc_sandboxed cat "${HOME}/.ssh/known_hosts"

# ── gh ──
tc_test "gh"
mkdir -p "${HOME}/.config/gh"
echo "fixture" > "${HOME}/.config/gh/hosts.yml"
__fixtures_created+=("${HOME}/.config/gh/hosts.yml" "${HOME}/.config/gh")
...
t "gh: ~/.ssh still blocked"
expect_fail "blocked" tc_sandboxed cat "${HOME}/.ssh/known_hosts"

# ── Toolchain isolation ──
tc_test "node"

t "node toolchain does NOT grant ~/.cargo access"
expect_fail "blocked" tc_sandboxed cat "${HOME}/.cargo/test"

t "node toolchain does NOT grant ~/.pyenv access"
expect_fail "blocked" tc_sandboxed cat "${HOME}/.pyenv/test"

# Clean up toolchain profiles
rm -f "${TMPDIR_RESOLVED}"/xclaude-tc-*-$$.sb
